...
export default class ItemList extends Component {
  ...
  render() {
    let birdName = [];
    const { onClick, answerId, answerState, count, isSuccess, rightIgitd } = this.props;
    ...
    return (
      <ul className="item-list list-group">
        {birdName.map((name, index) => (
          <li className={
            answerId === index + 1 || (!!isSuccess && rightId === index + 1)
              ? `${cls} ${status}`
              : cls
          }
            key={index} onClick={onClick} id={index + 1}>
